				...
				<div>
					...
					<div class="flex flex-wrap gap-2 mt-1">
						{#each alerts.slice(0, 5) as a}
							<span class="text-xs bg-amber-100 dark:bg-amber-800 text-amber-700 dark:text-amber-200 px-2 py-0.5 rounded font-medium">
								{a.nama} <b class="text-amber-900 dark:text-amber-100">{a.stok}</b>/{a.min_stok}
							</span>
						{/each}
						{#if alerts.length > 5}
							<span class="text-xs text-amber-500 font-medium">+{alerts.length - 5} lainnya</span>
						{/if}
					</div>
				</div>
			</div>
			<div class="flex items-center gap-2 shrink-0">
				<a href="/finance/{slug}/produk" class="text-xs font-bold text-amber-700 dark:text-amber-300 hover:underline whitespace-nowrap">
					Kelola Stok
				</a>
