<script>
    ...
    $: {
        managerLinks = [
            { label: 'Dashboard Keuangan', href: financeLink(''), icon: '📊' },
            ...
            { label: 'POS', href: financeLink('pos'), icon: '🛒' },
...
<div class="bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm" in:fade>
    <div class="flex items-center gap-2 mb-4">
        ...
        <div>
            <h3 class="text-sm font-bold text-slate-800 dark:text-slate-100">Manager Dashboard</h3>
            <p class="text-[10px] text-slate-500 dark:text-slate-400">Akses Penuh Manajemen</p>
        </div>
    </div>

    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
        {#each managerLinks as link}
            {#if isLinksValid}
                <a href={link.href} class="flex flex-col items-center gap-2 p-4 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all group">
                    ...
                    <span class="text-[10px] font-semibold text-slate-700 dark:text-slate-200 group-hover:text-purple-600 dark:group-hover:text-purple-400 text-center">{link.label}</span>
                </a>
            {:else}
